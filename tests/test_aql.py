...
def test_aql_query_management(db, bad_db, col, docs):
    ...
    cursor = db.aql.validate('FOR d IN {} RETURN d'.format(col.name))
    assert 'ast' in cursor
    assert 'bindVars' in cursor
    assert 'collections' in cursor
    assert 'parsed' in cursor
    ...
    db.begin_async(return_result=True).aql.execute('RETURN SLEEP(100)')
    db.begin_async(return_result=True).aql.execute('RETURN SLEEP(50)')

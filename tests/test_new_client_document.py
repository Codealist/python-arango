...
arango_client = ArangoClient(async_ready=True)
db_name = generate_db_name()
db = arango_client.create_database(db_name).result()
col_name = generate_col_name()
col = db.create_collection(col_name).result()
edge_col_name = generate_col_name()
edge_col = db.create_collection(edge_col_name, edge=True).result()
geo_index = col.add_geo_index(['coordinates']).result()
...
def test_insert():
    ...
    doc = doc1
    result = col.insert(doc, sync=True).result()
    assert result['_id'] == '{}/{}'.format(col.name, doc['_key'])
    assert result['_key'] == doc['_key']
    assert isinstance(result['_rev'], string_types)
    ...
    doc = doc2
    result = col.insert(doc, sync=False).result()
    assert result['_id'] == '{}/{}'.format(col.name, doc['_key'])
    assert result['_key'] == doc['_key']
    assert isinstance(result['_rev'], string_types)
    ...
    doc = doc3
    result = col.insert(doc, return_new=True).result()
    assert result['_id'] == '{}/{}'.format(col.name, doc['_key'])
    assert result['_key'] == doc['_key']
    assert isinstance(result['_rev'], string_types)
    ...
    doc = doc4
    result = col.insert(doc, return_new=False).result()
    assert result['_id'] == '{}/{}'.format(col.name, doc['_key'])
    assert result['_key'] == doc['_key']
    assert isinstance(result['_rev'], string_types)
    ...
    assert isinstance(col.insert(doc4).result(), DocumentInsertError)


def test_insert_many():
    # Test insert_many with default options
    results = col.insert_many(test_docs).result()
    ...
    results = col.insert_many(test_docs, sync=True).result()
    for result, doc in zip(results, test_docs):
        assert result['_id'] == '{}/{}'.format(col.name, doc['_key'])
        assert result['_key'] == doc['_key']
        assert isinstance(result['_rev'], string_types)
    ...
    results = col.insert_many(test_docs, sync=False).result()
    for result, doc in zip(results, test_docs):
        assert result['_id'] == '{}/{}'.format(col.name, doc['_key'])
        assert result['_key'] == doc['_key']
        assert isinstance(result['_rev'], string_types)
    ...
    results = col.insert_many(test_docs, return_new=True).result()
    for result, doc in zip(results, test_docs):
        assert result['_id'] == '{}/{}'.format(col.name, doc['_key'])
        assert result['_key'] == doc['_key']
        assert isinstance(result['_rev'], string_types)
    ...
    results = col.insert_many(test_docs, return_new=False).result()
    for result, doc in zip(results, test_docs):
        assert result['_id'] == '{}/{}'.format(col.name, doc['_key'])
        assert result['_key'] == doc['_key']
        assert isinstance(result['_rev'], string_types)
    ...
    assert isinstance(bad_col.insert_many(test_docs).result(),
                      DocumentInsertError)
...
def test_update():
    ...
    doc['val'] = {'bar': 2}
    doc = col.update(doc, merge=True).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    ...
    doc['val'] = {'baz': 3}
    doc = col.update(doc, merge=False).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    doc['val'] = None
    doc = col.update(doc, keep_none=True).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    doc['val'] = None
    doc = col.update(doc, keep_none=False).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    doc['val'] = 300
    doc = col.update(doc, return_new=True, return_old=True).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    doc['val'] = 400
    doc = col.update(doc, return_new=False, return_old=False).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    assert isinstance(col.update(doc, check_rev=True).result(),
                      DocumentRevisionError)
    assert col['1']['val'] == 400

    # Test update with sync
    doc['val'] = 600
    doc = col.update(doc, sync=True).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    doc['val'] = 700
    doc = col.update(doc, sync=False).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    assert isinstance(col.update(doc2).result(), DocumentUpdateError)
    assert '2' not in col
    assert col['1']['val'] == 700
    assert col['1']['_rev'] == current_rev

    # Test update in missing collection
    assert isinstance(bad_col.update(doc).result(), DocumentUpdateError)
...
def test_update_many():
    ...
    for doc in docs:
        doc['val'] = {'foo': 1}
    results = col.update_many(docs).result()
    ...
    for doc in docs:
        doc['val'] = {'bar': 2}
    results = col.update_many(docs, merge=True).result()
    ...
    for doc in docs:
        doc['val'] = {'baz': 3}
    results = col.update_many(docs, merge=False).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    for doc in docs:
        doc['val'] = None
    results = col.update_many(docs, keep_none=True).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    for doc in docs:
        doc['val'] = None
    results = col.update_many(docs, keep_none=False).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    for doc in docs:
        doc['val'] = 300
    results = col.update_many(docs, return_new=True, return_old=True).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    results = col.update_many(docs, check_rev=True).result()
    for result, key in zip(results, doc_keys):
    ...
    for doc in docs:
        doc['val'] = 600
    results = col.update_many(docs, sync=True).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    for doc in docs:
        doc['val'] = 700
    results = col.update_many(docs, sync=False).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    results = col.update_many([{'_key': '6'}, {'_key': '7'}]).result()
    for result, key in zip(results, doc_keys):
    ...
    assert isinstance(bad_col.update_many(docs).result(), DocumentUpdateError)


def test_update_match():
    # Test preconditions
    assert col.update_match({'val': 100}, {'foo': 100}).result() == 0

    # Set up test documents
    col.import_bulk(test_docs)

    # Test update single matching document
    assert col.update_match({'val': 200}, {'foo': 100}).result() == 1
    assert col['4']['val'] == 200
    assert col['4']['foo'] == 100

    # Test update multiple matching documents
    assert col.update_match({'val': 100}, {'foo': 100}).result() == 3
    for key in ['1', '2', '3']:
    ...
    assert isinstance(bad_col.update_match({'val': 100}, {'foo': 100})
...
def test_replace():
    ...
    # Test update with return_new and return_old
    doc['bar'] = 300
    doc = col.replace(doc, return_new=True, return_old=True).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    # Test update without return_new and return_old
    doc['baz'] = 400
    doc = col.replace(doc, return_new=False, return_old=False).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    doc['foo'] = 500
    doc['_rev'] = current_rev + '000'
    assert isinstance(col.replace(doc, check_rev=True).result(),
                      DocumentRevisionError)
    assert 'foo' not in col['1']
    assert col['1']['baz'] == 400
    ...
    doc['foo'] = 500
    doc = col.replace(doc, sync=True).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    doc['bar'] = 600
    doc = col.replace(doc, sync=False).result()
    assert doc['_id'] == '{}/1'.format(col.name)
    assert doc['_key'] == '1'
    assert isinstance(doc['_rev'], string_types)
    assert doc['_old_rev'] == current_rev
    ...
    assert isinstance(col.replace(doc2).result(), DocumentReplaceError)
    assert col['1']['bar'] == 600
    assert col['1']['_rev'] == current_rev

    # Test replace in missing collection
    assert isinstance(bad_col.replace(doc).result(), DocumentReplaceError)


def test_replace_many():
    current_revs = {}
    docs = [doc.copy() for doc in test_docs]
    col.insert_many(docs).result()

    # Test replace_many with default options
    for doc in docs:
        doc['foo'] = 200
        doc.pop('val')
    results = col.replace_many(docs).result()
    ...
    for doc in docs:
        doc['bar'] = 300
        doc.pop('foo')
    results = col.replace_many(docs, return_new=True, return_old=True).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    for doc in docs:
        ...
        doc.pop('baz')
        doc['_rev'] = current_revs[doc['_key']] + '000'
    results = col.replace_many(docs, check_rev=True).result()
    for result, key in zip(results, test_doc_keys):
        assert isinstance(result, DocumentRevisionError)
    for doc in col:
    ...
    for doc in docs:
        doc['foo'] = 500
    results = col.replace_many(docs, sync=True).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    for doc in docs:
        doc['bar'] = 600
        doc.pop('foo')
    results = col.replace_many(docs, sync=False).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    results = col.replace_many([{'_key': '6'}, {'_key': '7'}]).result()
    for result, key in zip(results, test_doc_keys):
    ...
    assert isinstance(bad_col.replace_many(docs).result(),
                      DocumentReplaceError)


def test_replace_match():
    # Test preconditions
    assert col.replace_match({'val': 100}, {'foo': 100}).result() == 0

    # Set up test documents
    col.import_bulk(test_docs)

    # Test replace single matching document
    assert col.replace_match({'val': 200}, {'foo': 100}).result() == 1
    assert 'val' not in col['4']
    assert col['4']['foo'] == 100

    # Test replace multiple matching documents
    assert col.replace_match({'val': 100}, {'foo': 100}).result() == 3
    for key in ['1', '2', '3']:
        assert 'val' not in col[key]
        assert col[key]['foo'] == 100
    ...
    assert isinstance(
        bad_col.replace_match({'val': 100}, {'foo': 100}).result(),
        DocumentReplaceError)
...
def test_delete():
    ...
    result = col.delete(doc2['_key']).result()
    ...
    result = col.delete(doc3, return_old=True).result()
    ...
    result = col.delete(doc4, sync=True).result()
    ...
    assert isinstance(col.delete(bad_doc, check_rev=True).result(),
                      ArangoError)
    ...
    # Test delete (document) with check_rev
    assert col.delete(doc4, ignore_missing=True).result() is False
    assert isinstance(col.delete(doc4, ignore_missing=False).result(),
                      ArangoError)
    assert len(col) == 1

    # Test delete with missing collection
    assert isinstance(bad_col.delete(doc5).result(), ArangoError)
    assert isinstance(bad_col.delete(doc5['_key']).result(), ArangoError)

    # Test delete with wrong user credentials
    err = bad_db.collection(col_name).delete(doc5).result()
    assert isinstance(err, DocumentDeleteError) \
        or isinstance(err, AsyncExecuteError) \
        or isinstance(err, BatchExecuteError)
...
def test_delete_many():
    ...
    results = col.delete_many(docs).result()
    for result, key in zip(results, test_doc_keys):
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
        assert isinstance(result['_rev'], string_types)
    ...
    col.import_bulk(docs)
    results = col.delete_many(docs, return_old=True).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    col.import_bulk(docs)
    results = col.delete_many(docs, sync=True).result()
    for result, doc in zip(results, docs):
        key = doc['_key']
        assert result['_id'] == '{}/{}'.format(col.name, key)
        assert result['_key'] == key
    ...
    for doc in docs:
        doc['_rev'] = current_revs[doc['_key']] + '000'
    results = col.delete_many(docs, check_rev=True).result()
    for result, doc in zip(results, docs):
        assert isinstance(result, DocumentRevisionError)
    assert len(col) == 5
    ...
    col.truncate()
    results = col.delete_many([{'_key': '6'}, {'_key': '7'}]).result()
    for result, doc in zip(results, docs):
        assert isinstance(result, DocumentDeleteError)
    assert len(col) == 0

    # Test delete_many with missing collection
    assert isinstance(bad_col.delete_many(docs).result(), DocumentDeleteError)
    assert isinstance(bad_col.delete_many(test_doc_keys).result(),
                      DocumentDeleteError)


def test_delete_match():
    # Test preconditions
    assert col.delete_match({'val': 100}).result() == 0
    ...
    assert '4' in col
    assert col.delete_match({'val': 200}).result() == 1
    assert '4' not in col

    # Test delete matching documents with sync
    assert '5' in col
    assert col.delete_match({'val': 300}, sync=True).result() == 1
    assert '5' not in col

    # Test delete matching documents with limit of 2
    assert col.delete_match({'val': 100}, limit=2).result() == 2
    assert [doc['val'] for doc in col].count(100) == 1

    assert isinstance(bad_col.delete_match({'val': 100}).result(),
                      DocumentDeleteError)
...
def test_count():
    ...
    assert col.count().result() == len(test_docs)

    assert isinstance(bad_col.count().result(), DocumentCountError)
...
def test_find():
    ...
    found = list(col.find({'val': 100}, offset=1).result())
    assert len(found) == 2
    for doc in map(dict, found):
        assert doc['_key'] in {'1', '2', '3'}
        assert doc['_key'] in col
    ...
    for limit in [3, 4, 5]:
        found = list(col.find({}, limit=limit).result())
    ...
    col.truncate()
    assert list(col.find({}).result()) == []
    assert list(col.find({'val': 100}).result()) == []
    assert list(col.find({'val': 200}).result()) == []
    assert list(col.find({'val': 300}).result()) == []
    assert list(col.find({'val': 400}).result()) == []

    # Test find in missing collection
    assert isinstance(bad_col.find({'val': 100}).result(), DocumentGetError)
...
def test_has():
    ...
    assert col.has('1').result() is True

    # Test has another existing document
    assert col.has('2').result() is True

    # Test has missing document
    assert col.has('6').result() is False

    # Test has with correct revision
    good_rev = col['5']['_rev']
    assert col.has('5', rev=good_rev).result() is True

    # Test has with invalid revision
    bad_rev = col['5']['_rev'] + '000'
    assert isinstance(col.has('5', rev=bad_rev, match_rev=True).result(),
                      ArangoError)
    ...
    assert isinstance(bad_col.has('1').result(), DocumentInError)
...
def test_get():
    ...
    result = col.get('1').result()
    assert result['_key'] == '1'
    assert result['val'] == 100

    # Test get another existing document
    result = col.get('2').result()
    assert result['_key'] == '2'
    assert result['val'] == 100

    # Test get missing document
    assert col.get('6').result() is None

    # Test get with correct revision
    good_rev = col['5']['_rev']
    result = col.get('5', rev=good_rev).result()
    assert result['_key'] == '5'
    assert result['val'] == 300

    # Test get with invalid revision
    bad_rev = col['5']['_rev'] + '000'
    assert isinstance(col.get('5', rev=bad_rev, match_rev=True).result(),
                      ArangoError)
    assert isinstance(col.get('5', rev='bad_rev').result(), ArangoError)
    ...
    # Test get with missing collection
    assert isinstance(bad_col.get('1').result(), DocumentGetError)
...
def test_get_from_db():
    ...
    # Test get existing document
    result = db.get_document(col_name + '/1').result()
    assert result['_key'] == '1'
    assert result['val'] == 100

    # Test get another existing document
    result = db.get_document(col_name + '/2').result()
    assert result['_key'] == '2'
    assert result['val'] == 100

    # Test get missing document
    assert db.get_document(col_name + '/6').result() is None

    # Test get with correct revision
    good_rev = db.get_document(col_name + '/5').result()['_rev']
    result = db.get_document(col_name + '/5', rev=good_rev).result()
    assert result['_key'] == '5'
    assert result['val'] == 300
...
def test_get_many():
    ...
    assert col.get_many(['6']).result() == []
    assert col.get_many(['6', '7']).result() == []
    assert col.get_many(['6', '7', '8']).result() == []

    # Test get_many existing documents
    result = col.get_many(['1']).result()
    result = clean_keys(result)
    assert result == [doc1]

    result = col.get_many(['2']).result()
    result = clean_keys(result)
    assert result == [doc2]

    result = col.get_many(['3', '4']).result()
    assert clean_keys(result) == [doc3, doc4]

    result = col.get_many(['1', '3', '6']).result()
    assert clean_keys(result) == [doc1, doc3]

    # Test get_many in empty collection
    col.truncate()
    assert col.get_many([]).result() == []
    assert col.get_many(['1']).result() == []
    assert col.get_many(['2', '3']).result() == []
    assert col.get_many(['2', '3', '4']).result() == []

    assert isinstance(bad_col.get_many(['2', '3', '4']).result(),
                      DocumentGetError)


def test_all():
    # Check preconditions
    assert len(list(col.export().result())) == 0

    # Set up test documents
    col.import_bulk(test_docs)

    # Test all with default options
    result = list(col.all().result())
    assert ordered(clean_keys(result)) == test_docs

    # Test all with a skip of 0
    result = col.all(skip=0).result()
    assert result.count() == len(test_docs)
    assert ordered(clean_keys(result)) == test_docs

    # Test all with a skip of 1
    result = col.all(skip=1).result()
    ...
    result = col.all(skip=4, limit=2).result()
    assert result.count() == 1
    assert len(list(result)) == 1
    for doc in list(clean_keys(list(result))):
        assert doc in test_docs
    ...
    assert isinstance(bad_col.all().result(), DocumentGetError)
...
def test_random():
    ...
    assert isinstance(bad_col.random().result(), DocumentGetError)
...
def test_find_near():
    ...
    result = col.find_near(latitude=1, longitude=1).result()
    assert [doc['_key'] for doc in result] == ['1', '2', '3', '4', '5']

    # Test find_near with limit of 0
    result = col.find_near(latitude=1, longitude=1, limit=0).result()
    assert [doc['_key'] for doc in result] == []

    # Test find_near with limit of 1
    result = col.find_near(latitude=1, longitude=1, limit=1).result()
    assert [doc['_key'] for doc in result] == ['1']

    # Test find_near with limit of 3
    result = col.find_near(latitude=1, longitude=1, limit=3).result()
    assert [doc['_key'] for doc in result] == ['1', '2', '3']

    # Test find_near with limit of 3 (another set of coordinates)
    result = col.find_near(latitude=5, longitude=5, limit=3).result()
    assert [doc['_key'] for doc in result] == ['5', '4', '3']

    # Test random in missing collection
    assert isinstance(bad_col.find_near(latitude=1, longitude=1,
                                        limit=1).result(), DocumentGetError)

    # Test find_near in an empty collection
    col.truncate()
    result = col.find_near(latitude=1, longitude=1, limit=1).result()
    assert list(result) == []
    result = col.find_near(latitude=5, longitude=5, limit=4).result()
    assert list(result) == []
...
def test_find_in_range():
    ...
    result = col.find_in_range(field='val', lower=100, upper=200).result()
    ...
    result = col.find_in_range('val', 100, 200, inclusive=False).result()
    assert [doc['_key'] for doc in result] == []

    # Test find_in_range without inclusive
    result = col.find_in_range('val', 100, 300, inclusive=False).result()
    assert [doc['_key'] for doc in result] == ['4']

    # Test find_in_range in missing collection
    assert isinstance(bad_col.find_in_range(field='val', lower=100,
                                            upper=200, offset=2).result(),
                      DocumentGetError)
...
def test_find_in_radius():
    ...
    result = list(col.find_in_radius(3, 3, 10, 'distance').result())
    for doc in result:
        assert 'distance' in doc

    # Test find_in_radius in missing collection
    assert isinstance(bad_col.find_in_radius(3, 3, 10, 'distance').result(),
                      DocumentGetError)
...
def test_find_in_box():
    ...
    assert isinstance(bad_col.find_in_box(
            latitude1=0,
            longitude1=0,
            latitude2=6,
            longitude2=3,
...
def test_import_bulk():
    ...
    result = col.import_bulk(test_docs).result()
    ...
    result = col.import_bulk(test_docs, details=False, sync=True).result()
    assert result['created'] == 5
    assert result['errors'] == 0
    assert result['empty'] == 0
    assert result['updated'] == 0
    ...
    assert isinstance(col.import_bulk([doc1, doc1], halt_on_error=True)
                      .result(), DocumentInsertError)
    assert len(col) == 0

    # Test import bulk duplicates without halt_on_error
    result = col.import_bulk([doc2, doc2], halt_on_error=False).result()
    ...
    assert isinstance(bad_col.import_bulk([doc3, doc4], halt_on_error=True)
                      .result(),
                      DocumentInsertError)
    assert len(col) == 1

    # Test import bulk with overwrite
    result = col.import_bulk([doc3, doc4], overwrite=True).result()
    ...
    col.insert(old_doc)
    result = col.import_bulk([new_doc], on_duplicate='error').result()
    assert len(col) == 1
    ...
    result = col.import_bulk([new_doc], on_duplicate='ignore').result()
    assert len(col) == 1
    assert result['created'] == 0
    assert result['errors'] == 0
    assert result['empty'] == 0
    ...
    result = col.import_bulk([new_doc], on_duplicate='update').result()
    assert len(col) == 1
    assert result['created'] == 0
    assert result['errors'] == 0
    assert result['empty'] == 0
    ...
    col.truncate()
    col.insert(old_doc)
    result = col.import_bulk([new_doc], on_duplicate='replace').result()
    assert len(col) == 1
    assert result['created'] == 0
    assert result['errors'] == 0
    assert result['empty'] == 0
